...
def _get_matlab_engine():
    """Lazily connect to MATLAB engine only when needed."""
    if not USE_MATLAB:
        return None
    import matlab.engine
    if not hasattr(_get_matlab_engine, '_engine'):
        _get_matlab_engine._engine = matlab.engine.connect_matlab()
    return _get_matlab_engine._engine

eng = None  # Will be set lazily via _get_matlab_engine() when USE_MATLAB is True
...
def compute_prior_cdf(r, eta, method='gamma_cdf', n_samples = 1000, tail_bound = 0.001, tail_percent = 0.1, scale = 1, use_matlab=None, eng=None, debug=True, enforce_assert=True, return_assert=False, return_xs=False):
    if use_matlab is None:
        use_matlab = USE_MATLAB
    if eng is None and use_matlab:
