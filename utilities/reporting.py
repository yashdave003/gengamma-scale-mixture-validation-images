...
from utilities.testing import variance_prior, kurtosis_prior
...
def add_hull_and_kurt(master_df, rEtaKsstats_dict, GROUP='group', debug=False):
    ...
    for group in groups:
        ...
        if master_df_copy.loc[group, "total_samples"] < 10:
            ...
            full_df['kurt'] = full_df.apply(lambda row : kurtosis_prior(r=row['r'], eta=row['eta']), axis=1)
...
def load_main_df():
    """Load the main results dataframe. Call this instead of relying on module-level side effects."""
